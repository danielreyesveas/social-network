...
export default class CreateData implements Seeder {
	public async run(_: any, connection: Connection): Promise<any> {
		...
		const elliot = await User.findOne({ username: "elliot" });
		const mr_robot = await User.findOne({ username: "mr_robot" });
		...
		await connection
			...
			.values([
				...
				{
					body:
						"This is so bad, I dont know why im laughing Hahahaha!!",
					post: post9,
					user: mr_robot,
					identifier: "VxnlwvEx",
		...
		await connection
			...
			.into(Vote)
			.values([
				{ value: 1, user: elliot, post: post9 },
				{ value: 1, user: mr_robot, post: post9 },
				{ value: 1, user: mr_robot, post: post8 },
				{ value: 1, user: elliot, comment: comment1 },
				{ value: 1, user: mr_robot, comment: comment1 },
				{ value: 1, user: elliot, comment: comment2 },
			])
			.execute();
	}
}
